...
from net.basic_models.BilinearBlock import BilinearClassifyBlock
...
class I3D(torch.nn.Module):
    def __init__(self,
        ...
        self.se_layer_s = SELayer3D(480,32)
        self.se_layer_m = SELayer3D(832,16)
        self.se_layer_l = SELayer3D(1024,8)

        self.bilinear = BilinearClassifyBlock(1024,self.num_classes)
    ...
    def forward(self, inp):
        ...
        out = self.se_layer_m(out)

        out = self.maxPool3d_5a_2x2(out)
        out = self.mixed_5b(out)
        out = self.mixed_5c(out)

        out = self.se_layer_l(out)
        path = out
        ...
        out = out.squeeze(3)
        out = out.mean(2)
        bilinear_path = self.bilinear(path)
        out += bilinear_path
